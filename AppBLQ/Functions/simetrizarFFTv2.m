...
function [Symmetric] = simetrizarFFTv2(Maps,Type)

[Filas, Columnas] = size(Maps{1});
...
if mod(Filas,2)==0 && mod(Columnas,2) == 0
    Maps = cellfun(@(x) x([1:end 1],[1:end 1]),Maps,UniformOutput=false);
end
switch Type
    case 'Vertical'
        Symmetric = cellfun(@(x) 0.5*(x+flipud(x)),Maps,UniformOutput=false);
